    ...
    IRedisSerDes serDes) : IRedisListService
{
    ...
    public Either<Error, Unit> Prepend<T>(string key, params T[] values) =>
        Wrap(() => _database.ListLeftPush(key, [.. values.Select(_serDes.Serialize)]), ToUnit);
    ...
    public Task<Either<Error, Unit>> PrependAsync<T>(string key, params T[] values) =>
        WrapAsync(() => _database.ListLeftPushAsync(key, [.. values.Select(_serDes.Serialize)]), ToUnit);
    ...
    public Either<Error, Unit> Append<T>(string key, params T[] values) =>
        Wrap(() => _database.ListRightPush(key, [.. values.Select(_serDes.Serialize)]), ToUnit);
    ...
    public Task<Either<Error, Unit>> AppendAsync<T>(string key, params T[] values) =>
        WrapAsync(() => _database.ListRightPushAsync(key, [.. values.Select(_serDes.Serialize)]), ToUnit);
