    ...
    IRedisSerDes serDes) : IRedisSortedSetService
{
    ...
    private Either<Error, T[]> Combine<T>(string[] keys, SetOperation operation) =>
        Wrap(() => _database.SortedSetCombine(operation, keys.Select(k => (RedisKey)k).ToArray()).Select(_serDes.Deserialize<T>).Filter().ToArray());
    ...
    private Task<Either<Error, T[]>> CombineAsync<T>(string[] keys, SetOperation operation) =>
        WrapUnsafeAsync(() => _database.SortedSetCombineAsync(operation, keys.Select(k => (RedisKey)k).ToArray()), vs => vs.Select(_serDes.Deserialize<T>).Filter().ToArray());
}
